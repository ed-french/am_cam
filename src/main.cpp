...
// ---------------------------------------------------------------
  ...
  bool sendRGBfile = 1;                                  // if set '/rgb' will send the rgb data as a file rather than display some on a HTML page
...
void handleRoot() {
  ...
  // Action any button presses or settings entered on web page
    ...
    // if button3 was pressed (toggle flash LED)
      ...
      if (server.hasArg("flashfull")) {
        illuminationBrightness(255);          // turn led on dim
        if (serialDebug) Serial.println("flash on full");
      }
        if (server.hasArg("flashoff")) {
        illuminationBrightness(0);          // turn led on dim
        if (serialDebug) Serial.println("flash off");
      }
